...
def route_as_layer(task, provider, profile, optimize, directions):
    ...
    if not provider["key"] and (provider["base_url"].startswith("https://api.openrouteservice.org") or provider["base_url"].startswith("https://api.heigit.org")):
    ...
    try:
        ...
        if optimize:
            ...
            if (
                ...
                logger.log(msg, 0)
                raise exceptions.EmptyLayerError()
